...
**Estado general:** G0, Catálogo C1–C4, D1, Q1, P1, Library Red, S1, S2, S2.1, S2.2 y L1 entregados; L2 implementada y validada localmente mediante el issue #49, con entrega Git pendiente

## Colección local y offline L2 — issue #49

- El [issue #49 — L2: implementar Colección local y offline](https://github.com/JFrancoG/MangaLibrary/issues/49) no tenía issues, PR o ramas equivalentes. La rama local `codex/49-l2-offline-collection` partió de `main@3fe8b236f68d08b2174cb907f2c332061e49a812`, limpio y sincronizado con `origin/main`. El propietario aprobó las seis decisiones y el plan el 2026-09-01; la autorización cubre implementación, tests, documentación y validación, pero no commit, push, PR, merge, cierre del issue ni borrado de la rama.
...
La implementación permanece local y sin commit en `codex/49-l2-offline-collection`; la rama no tiene upstream ni PR y el issue #49 continúa abierto. Commit, push, PR, merge, cierre del issue y borrado de rama requieren una autorización posterior. R1, R2 y las capacidades Deluxe siguen siendo unidades separadas.
...
La lista de capacidades de la SDD 00 es una puerta de aceptación, no un orden de implementación. El orden operativo parte de dos decisiones ya aprobadas: la colección local se identifica por **usuario + manga** y no existe una política de colección anónima. S1, S2, S2.1, S2.2 y L1 están entregados; L2 está implementada y validada localmente mediante el issue #49, con entrega pendiente. La secuencia completa es:
...
6. **L2 — Colección local y offline, implementada y validada localmente mediante el issue #49.** `@Query` queda restringida a la identidad activa y excluye tombstones; Colección posee navegación independiente y alta, edición y eliminación mediante la ruta semántica de L1. El esquema V2 añade presentación offline con migración lightweight, y tomos, lectura, colección completa, tombstones y aislamiento A/B sobreviven a reapertura sin red. La entrega Git continúa pendiente.
...
- L1 entrega mediante la PR #44 `ModelContainer`, esquema V1, modelos SwiftData, outbox y primera mutación atómica. L2 materializa localmente en el issue #49 el esquema V2, `@Query`, presentación offline y UI de Colección; su entrega Git sigue pendiente. No existen todavía worker ni sincronización remota.
