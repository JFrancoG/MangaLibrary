...
**Estado general:** G0, Catálogo C1–C4, D1, Q1, P1, Library Red, S1, S2, S2.1, S2.2, L1, L2, R1 y las correcciones #55 y #58 entregadas; R2.1 está implementado y validado localmente en el issue #57, preservado en un commit local y sin entrega remota ni prueba funcional live

## R2.1 — POST y reconciliación conservadora de outbox — issue #57

- El [issue #57 — R2: implementar envío y reconciliación de outbox](https://github.com/JFrancoG/MangaLibrary/issues/57) se abrió después de comprobar que no existía otro issue, PR o rama equivalente. La rama local `codex/57-r2-outbox-sync` parte de `main@80ac1bb665c4a4cfc380b3d8617aa276ced83cc0`, limpio y sincronizado con `origin/main` en el preflight aprobado. El corte validado se preservó en el commit local `d4b86b5` antes de integrar la entrega #58; push, PR, merge remoto, cierre y borrado de esta rama continúan sin autorización.
- La decisión del propietario elimina la ambigüedad semántica de los paths individuales: `{id}` es `Manga.ID` (`Int64`) y el transporte futuro escribirá sus dígitos decimales en el parámetro `string`. El UUID de la entrada remota no forma el path. El corte local lo registró en SDD 04 v1.18; la integración con la autoridad JWT de #58 eleva la especificación combinada a v1.19. La caracterización OpenAPI conserva la discrepancia machine-readable como deuda del contrato y la aceptación real de GET/DELETE individual sigue pendiente.
...
| Xcode MCP — `ReleaseGate` | 359/359 resultados aprobados en iPhone 17 Simulator con iOS 27, sin fallos, skips, expected failures o casos no ejecutados. El plan `Integration` continúa mostrando cero tests habilitados pese a las tags existentes; no se presenta como gate independiente, pero sus suites están habilitadas y ejecutadas dentro de `ReleaseGate`. |
...
- El [issue #58 — Auth: migrar a JWT único compatible con Colección](https://github.com/JFrancoG/MangaLibrary/issues/58) se abrió sin duplicados y permanece abierto. La rama `codex/58-jwt-collection-auth` parte de `main@80ac1bb665c4a4cfc380b3d8617aa276ced83cc0`, se implementó en un worktree aislado para preservar R2 y se entregó mediante la [PR #59](https://github.com/JFrancoG/MangaLibrary/pull/59).
...
8. **R2 — envío y reconciliación de outbox, en curso mediante el issue #57.** La decisión del propietario del 2 de septiembre fija `{id}` como `Manga.ID` `int64` serializado en decimal; el UUID de entrada no forma el path y la discrepancia `string` queda como deuda contractual. El primer corte reutiliza el GET completo R1 e implementa POST y procesamiento conservador de intenciones no tombstone. GET/DELETE individual, retry, `blockedAuth`, rechazo/reversión y resolución manual permanecen en cortes posteriores. Las pruebas automatizadas no alcanzan producción; la caracterización live de los paths individuales será una validación manual separada y autorizada.
