...
**Estado general:** G0, Catálogo C1–C4, D1, Q1, P1, Library Red, S1, S2, S2.1, S2.2, L1, L2, R1 y la corrección crítica #55 entregados; R2.1 está implementado y validado localmente en el issue #57, con autorización para preservarlo en un commit local antes de integrar #58 y sin prueba funcional live

## R2.1 — POST y reconciliación conservadora de outbox — issue #57

- El [issue #57 — R2: implementar envío y reconciliación de outbox](https://github.com/JFrancoG/MangaLibrary/issues/57) se abrió después de comprobar que no existía otro issue, PR o rama equivalente. La rama local `codex/57-r2-outbox-sync` parte de `main@80ac1bb665c4a4cfc380b3d8617aa276ced83cc0`, limpio y sincronizado con `origin/main` en el preflight aprobado. El propietario autorizó el commit local del corte validado para integrar después #58; push, PR, merge remoto, cierre y borrado de esta rama continúan sin autorización.
- La decisión del propietario elimina la ambigüedad semántica de los paths individuales: `{id}` es `Manga.ID` (`Int64`) y el transporte futuro escribirá sus dígitos decimales en el parámetro `string`. El UUID de la entrada remota no forma el path. SDD 04 v1.18 y la caracterización OpenAPI registran la discrepancia machine-readable como deuda del contrato; la aceptación real de GET/DELETE individual sigue pendiente.
...
Codex no realizó login, petición funcional autenticada ni escritura live. Por
