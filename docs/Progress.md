...
**Estado general:** G0, Catálogo C1–C4, D1, Q1, P1, Library Red, S1, S2, S2.1, S2.2, L1, L2, R1 y las correcciones #55 y #58 entregadas; R2.1 está preservado en un commit local y R2.2 está implementado y validado en el worktree del issue #57, con aceptación live multidispositivo de DELETE/reconciliación y sin entrega remota

## R2.2 — GET/DELETE individual y envío de tombstones — issue #57

- El propietario autorizó ampliar el [issue #57 — R2: implementar envío y reconciliación de outbox](https://github.com/JFrancoG/MangaLibrary/issues/57) con ambos endpoints individuales y la interacción de borrado. El trabajo continúa en `codex/57-r2-outbox-sync`: R2.1 permanece preservado en el commit local `d4b86b5` y este corte R2.2 no se ha commiteado, pusheado ni entregado. El issue sigue abierto.
...
| Xcode MCP — build y diagnósticos | Build-for-testing posterior aprobado en 2,037 s. El log estructurado devuelve 0 warnings y 0 errores; `CollectionEditorView.swift` y `MangaLibraryUITests.swift` devuelven 0 diagnósticos, igual que la revisión focal anterior del resto del corte. |
...
- L1 entrega mediante la PR #44 `ModelContainer`, esquema V1, modelos SwiftData, outbox y primera mutación atómica. L2 entrega mediante la PR #50 el esquema V2, `@Query`, presentación offline y UI de Colección. R1 entrega mediante la PR #54 la lectura e importación remota con reconciliación local-first. R2 se trabaja en #57: R2.1 incorpora POST y vaciado seguro de intenciones no tombstone; R2.2 implementa GET/DELETE individual y tombstones y cuenta con aceptación live multidispositivo, aunque conserva la deuda contractual descrita en su evidencia. Retry/backoff, `blockedAuth`, rechazo/reversión y resolución manual siguen pendientes.
