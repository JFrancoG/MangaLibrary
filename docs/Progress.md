...
**Estado general:** G0, Catálogo C1–C4, D1, Q1, P1, Library Red, S1, S2 y S2.1 entregados; S2.2 está implementado y validado localmente en el issue #39, pendiente de entrega; L1 sigue siendo el siguiente corte y no se ha iniciado
...
- Tracker: [GitHub Issue #39 — S2.2: completar Cuenta, alta HTTP y sesión segura](https://github.com/JFrancoG/MangaLibrary/issues/39), abierto y pendiente de entrega. Su descripción ya reconcilia el alcance ampliado aprobado durante la validación.
- Rama local: `codex/39-s22-credential-forms`, creada desde `main@289fff7ad54c50b5459a9d834bcdec97875694b5`. El trabajo permanece sin commit, push o PR de entrega.
...
El plan físico acredita cuatro recorridos UI sintéticos, incluidos los dos de Cuenta. La observación manual adicional del propietario acredita el status `201` de un alta real y un login live posterior correcto; no caracteriza el body, el almacenamiento interno ni el logout. La implementación no valida AutoFill o gestor de contraseñas real, tecnologías de asistencia, interacción runtime en landscape o multitarea. `SecureField` no expone a SwiftUI la posición de cursor o selección, por lo que el gate acredita contenido y foco modelado al alternar, no una posición concreta del cursor. Esas exclusiones no se presentan como evidencia superada. S2.2 continúa abierto y pendiente de las acciones de entrega que autorice el propietario.
...
La lista de capacidades de la SDD 00 es una puerta de aceptación, no un orden de implementación. El orden operativo parte de dos decisiones ya aprobadas: la colección local se identifica por **usuario + manga** y no existe una política de colección anónima. S1, S2 y S2.1 están entregados; S2.2 está implementado y validado localmente, pendiente de entrega; L1 continúa después. La secuencia completa es:
...
- S2.2 materializa localmente la validación y presentación de credenciales en el issue #39; continúa pendiente de entrega y no incorpora persistencia de producto.
