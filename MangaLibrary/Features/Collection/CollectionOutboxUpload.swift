...
extension CollectionMutationActor {
    ...
    /// Removes earlier confirmed rows for the same user and manga inside the caller's authorized transaction.
    ///
    /// The caller must have confirmed the supplied operation. This helper neither saves nor opens a transaction.
    func removeOlderConfirmedUploads(than confirmedOperation: CollectionOutboxOperation) throws(any Error) {
