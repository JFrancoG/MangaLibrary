...
El siguiente corte de producto es **S1 — identidad y sesión dual**. Debe obtener una identidad estable antes de exponer Colección porque el modelo aprobado se particiona por usuario y manga y no define una colección anónima. Después llegarán el alta de usuario, SwiftData local con outbox atómica, la experiencia offline de Colección y la sincronización remota. La [hoja de ruta Advanced](docs/Progress.md#hoja-de-ruta-advanced) delimita esos cortes; autenticación, persistencia de producto, Colección, sincronización, WidgetKit y watchOS continúan sin implementar.
